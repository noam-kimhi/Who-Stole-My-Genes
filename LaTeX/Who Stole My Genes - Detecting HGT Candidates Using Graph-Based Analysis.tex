...
\begin{document}
	...
	For each unordered species pair, we computed the median and median absolute deviation (MAD) of the edge Jaccard similarity distribution and used these values to define robust $z$-scores, which were assigned only for species pairs represented by at least 30 edges. Pairs with fewer observations were excluded from the downstream $z$-based node and component summaries. \\ \\
	Within each connected component of the pruned graph, taxonomic mixing was quantified using the Shannon entropy\cite{ShannonEntropy} of species labels among its member proteins. Anomaly concentration was defined as the fraction of edges with $z\ge3$. \\ \\
	...
	\end{equation*}
	\noindent
	Where $\Phi(C_u)$ captures component-level taxonomic mixing and anomaly concentration, $A(u)$ aggregates normalized similarity scores, and $T(u)$ summarizes cross-species connectivity and bridge-like topology. To stabilize scale differences across components, aggregation was performed in log-space, meaning high scores arise only when anomaly and structural evidence co-occur. 
	...
	Using \hyperref[subsec:method1]{method 1}, we identified EptC as a candidate HGT event. EptC is an enzyme that adds a postive molecule (PEA) to the bacterial outer membrane, therefore reducing the net negative charge of the membrane surface. Antimicrobial peptides produced by the host are mostly positively charged. By decreasing the membrane’s negative charge, the enzyme discourages entrance of antimicrobial peptides and mitigtes their immunological effect. This provides a selective advantage to the bacterium. EptC is a known HGT-associated gene \cite{Zinkle2025}.\\
	...
	\subsection{Summary}
	In this project, we asked whether graph-based analysis of cross-species protein similarity can help reveal candidate horizontal gene transfer (HGT) events across bacterial genomes. Our original goal was not to replace classical phylogenetic approaches, but to develop a lightweight and interpretable framework that uses sparse similarity graphs, taxonomic context and anomaly scoring to prioritize candidate genes for further study.\\
	Overall, our results suggest that graph-based representations are indeed useful for this task. Accross both methodologies, we identified candidates whose similarity patterns deviate from the background expectation of species-level clustering --- EptC and ymdF using \hyperref[subsec:method1]{method 1}, and Glucose-1-phosphate thymidylyltransferase, 30S ribosomal protein S10, and ribosomal proteins S21/S12 using \hyperref[subsec:method2]{method 2}. Together, these findings support the idea that HGT candidates can be exposed through graph structure, especially when high similarity appears accross taxonomically distant species.
	
	\subsection{Analysis}
	The main conclusion from our results is that graph analysis provides meaningful information regarding HGT events. Our research question asked how graph-based analysis can reveal candidate HGT events, and the answer emerging from this project is that unusual edges become much more informative when viewed in their broader network context.\\\\
	In \hyperref[subsec:method1]{method 1}, the graph was constructed after clustering proteins into homologous groups, which made the problem computationally manageable and biologically focused. Within these groups, suspicious edges connected proteins that were both highly similar and taxonomically distant. The additional node-level aggregation step was important: instead of flagging a single surprising pair, we prioritized proteins that repeatedly participated in such anomalous relationships. The phylogenetic follow-up further strengthened this interpretation, since the inferred gene relationships did not fully match th expected species relationships.\\\\
	\hyperref[subsec:method2]{Method 2} complements this by showing that even without explicit alignments, large-scale graph screening can still recover biologically interesting signals. The use of $k$-mer based Jaccard-similarity, pruning, normalization and component-aware scoring allowed us to move from a very large candidate space to a relatively small set of high-interest proteins and connected components. Importantly, the high-scoring components were not merely dense graphs --- they also displayed taxonomic mixing.\\\\
	...
	A major strength of the project is that the two methodologies address complementary approaches. The fact that both methods identified plausible HGT-related candidates suggests that the graph-based perspective is robust across different computational formulations.\\
	However, the project has several important limitations.\\
	First, similarity is not equivalent to horizontal transfer. This was already recognized at the proposal stage as a central risk, and it remained the main conceptual limitation throughout the project. Our pipeline therefore identifies candidates, not confirmed transfers.\\
	Second, taxonomic distance in our framework is coarse. It is useful as a prior for distinguishing expected within-lineage similarity from more surprising cross-lineage similarity, but is does not fully model evolutionary history.\\
	Third, the dataset design introduces limitations. The two methodologies used different datasets, which reduced direct comparability between them. In \hyperref[subsec:method1]{method 1}, the number of proteomes was relatively small, allowing deeper per-cluster analysis but limiting taxonomic breadth. In \hyperref[subsec:method2]{method 2}, broader coverage improved scale, but selecting at most two assemblies per species introduced redundancy in some cases and may also have excluded informative diversity in others.\\
	Fourth, the scoring thresholds were partly heuristics. Thresholds were chosen to make the graph informative, but different parameter choices could change the ranked candidates.\\\\
	Regarding difficulties we have encountered, one practical challenge was balancing biological realism with computational feasibility. Classical HGT detection can become very expensive very quickly, especially when pairwise comparisons scale quadratically. This motivated the divide and conquer designs of both methods. Another challenge was distinguishing conserved proteins from truly anomalous cross-species similarities. Our response was to combine similarity with taxonomic context and graph-level aggregation rather than relying on any single local statistic.
	
	\subsection{Future Work}
	The most important next step would be to create stronger validation by comparing our ranked candidates against curated HGT benchmarks or predictions from established HGT tools. This would allow quantitative evaluation of precision and recall.\\
	A second direction is scaling. For \hyperref[subsec:method2]{method 2} in particular, sketching approaches such as MinHash \cite{Ondov2016} could further reduce the cost of large-scale similarity screening.\\
	A third direction is unifying the two methodologies into a single hierarchical pipline. For example, one could first apply \hyperref[subsec:method2]{method 2}'s alignment-free graph construction as a broad screening layer, then pass selected components or candidate proteins into \hyperref[subsec:method1]{method 1}'s alignment-based pipeline for finer analysis and phylogenetic inspection.\\
	Finally, future work could also move beyond bacterial datasets and ask whether similar graph-based anomaly patterns can help identify transfer across broader ecological or taxonomic boundaries.
